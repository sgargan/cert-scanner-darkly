...
func (assert *MetricsServerTests) TestPortOpened() {
...
func (assert *MetricsServerTests) TestPprofEnabled() {
...
func (assert *MetricsServerTests) TestTiming() {
...
func (assert *MetricsServerTests) TestThresholdsRefreshed() {
...
func (assert *MetricsServerTests) withServer(port, handler func(*PrometheusMetrics, *consulMock.MockConsulServer)) {
...
func (assert *MetricsServerTests) getMetrics(metricsServer *PrometheusMetrics, pattern string) map[string]float64 {
	url := fmt.Sprintf("http://%s/metrics", metricsServer.listenerAddress)
	response, statusCode, err := requests.Get(url, nil)
	assert.NoError(err)
	assert.Equal(http.StatusOK, statusCode)
